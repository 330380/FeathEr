...
const pages = [
  {
    ...
    expected: ["一个轻量的在线工具集合", "接口测试", "轻量性能测试"],
  },
  {
  ...
  {
    url: "https://330380.github.io/FeathEr/performance.html",
    expected: ["轻量性能测试", "平均耗时", "请求总次数"],
  },
];
    ...
    https
      .get(page.url, { headers: { "Cache-Control": "no-cache" } }, (response) => {
        ...
        response.on("end", () => {
          ...
          assert(!body.includes("涓€涓"), "public page contains mojibake");
          resolve();
        });
      })
      .on("error", (error) => {
